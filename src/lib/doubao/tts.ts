...
function buildAdditions(silenceDuration: number, contextTexts?: string[]): string {
  const additions: Record<string, unknown> = {
    silence_duration: clampRange(silenceDuration, 0, 30000),
    disable_markdown_filter: true,
  };

  if (contextTexts?.length) {
    additions.context_texts = [contextTexts[0]];
  }
...
export function buildTtsPayload(text: string, options: TTSOptions = {}) {
  ...
  const silence_duration = clampRange(options.silence_duration ?? 0, 0, 30000);
  ...
  const contextTexts = isTts20Voice(voice_type)
    ? (options.context_texts?.length ? options.context_texts : [DEFAULT_CONTEXT_TEXT])
    : undefined;
  ...
  const req_params: Record<string, unknown> = {
    ...
    speaker: voice_type,
    audio_params,
    additions: buildAdditions(silence_duration, contextTexts),
  };
